//Прелоадер для сайта
@@include('preLoader.js');

// в myLib лежат кастомные скрипты
@@include('myLib.js');

// Подключаем все скрипты
@@include('tabs.js');

// Скролл шапки и добавлении стилей с position: fixed.
@@include('headerScroll.js');

// Кнопка которая скроллит "домой"
@@include('buttonUp.js');

// Скрипт схемы (Принципиальная схема установки)
@@include('scheme.js');

// Скрипт кнопки "читать далее"
@@include('readMoreBtn.js');

// Popup меню
@@include('popup.js');

// Скролл к секциям
@@include('scrollTo.js');

// Бибилиотека с формами
@@include('common.js');

// Кастомная настройка плагина ToolTips, подключаем через CDN в html/_js.html
@@include('toolTips.js');

// Подключаем scrollTo
@@include('jquery.scrollTo.js');
@@include('jquery.scrollTo.min.js');

// Скрипт для переворачивания карточек
@@include('flipCards.js');

